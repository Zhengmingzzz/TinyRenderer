...
Object* AssetManager::load(const GUID& guid){
    ...
    TinyRenderer::io::from_json(res_json, var);
...
void AssetManager::load_by_path(const std::filesystem::path& file_path, rttr::instance obj) {
    ...
    TinyRenderer::io::from_json(res_json, obj);
}
...
bool AssetManager::save(Object *target_object_ptr) {
    ...
    // 开始序列化
    json resource_json = TinyRenderer::io::to_json(target_object_ptr);
...
json AssetManager::read_json_from_file(const std::filesystem::path& file_path) {
    ...
    // 检查文件是否成功打开
    if (!infile.is_open()) {
        std::cerr << file_path << " can not find" << std::endl;
        infile.close();
        return false;
    }
