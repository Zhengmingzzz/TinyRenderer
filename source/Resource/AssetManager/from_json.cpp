...
namespace TinyRenderer {
    namespace io {
        variant extract_basic_type(const json& j, const rttr::type& t);
        ...
        void write_array_recursively(variant_sequential_view &view, json &json_array_value)
        {
            ...
            for(size_t i = 0; i < json_array_value.size(); i++)
            {
                ...
                // 处理基本类型情况
                    ...
                    variant extracted_value = extract_basic_type(json_index_value, array_type);
        ...
        variant extract_value(json::iterator &itr, const type& t) {
            ...
            variant extracted_value = extract_basic_type(json_value, t);
                ...
                if(json_value.is_object()) {
                    ...
                    if (t.is_sequential_container()) {
                        ...
                        if (var_array.is_valid()) {
                            ...
                            write_array_recursively(seq_view, json_value);
                        ...
                        if (var_array.is_valid()) {
                            ...
                            write_associative_view_recursively(associate_view, json_value);
                            extracted_value = var_array;
                        }
                    }
                }
        ...
        void write_associative_view_recursively(variant_associative_view &view, json &json_array_value)
        {
            ...
            for(size_t i = 0; i < json_array_value.size(); i++)
            {
                ...
                if(json_index_value.is_object()) {
                    ...
                    {
                        // 显式处理 GUID 键或值
                        variant key_var = extract_value(key_itr, key_type);

                        variant value_var = extract_value(value_itr, value_type);
                ...
                {
                    variant extracted_value = extract_basic_type(json_index_value, key_type);
        ...
        void from_json_recursively(instance obj2, json& source_json)
        {
            ...
            for(auto& prop : prop_list)
            {
                ...
                if (!source_json.contains(prop_name) || prop.is_readonly())
                    continue;
                ...
                // 处理基础类型
                    ...
                    variant extracted_value = extract_basic_type(json_value, prop_type);
