...
namespace TinyRenderer {
    namespace io {
        ...
        void write_array_recursively(variant_sequential_view &view, json &json_array_value, bool is_guidToObject)
        {
            ...
            rttr::type elem_type = view.get_value_type();
            for(size_t i = 0; i < json_array_value.size(); i++)
            {
                ...
                // 处理Object*类型，直接进行反序列化
                else if (elem_type.is_derived_from(rttr::type::get<Object>()) && elem_type.is_pointer()) {
                    // 查看序列化标志是否要反序列化
                    if (!is_guidToObject)
                        return;
                    variant var = AssetManager::get_instance().load_variant(GUID(json_index_value.get<string>()));
                    Level* le_ptr = var.get_value<Level*>();
                    bool b = view.set_value(i, var);
                }
                // 处理基本类型情况
                else
                {
            ...
            for(auto& prop : prop_list)
            {
                ...
                if(json_value.is_array()) {
                    ...
                    if (value_type.is_sequential_container()) {
                        ...
                        write_array_recursively(view, json_value, prop.get_metadata(PROPERTY_FLAG_GUIDTOOBJECT).to_bool());
                    }
